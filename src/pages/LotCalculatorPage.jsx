    ...
    <div ref={panelRef} className="absolute z-50 left-0 right-0 mt-2 rounded-2xl overflow-hidden animate-fadeIn flex flex-col" style={{ background: 'rgba(15,17,21,0.98)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(16px)', maxHeight: 420 }}>
      ...
      <div className="p-3 pb-0">
        <div className="relative">
          ...
          <input ref={inputRef} type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search 400+ instruments..." className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm bg-kmf-panel text-kmf-text-primary outline-none placeholder:text-kmf-text-tertiary border border-white/5 focus:border-kmf-accent/30 transition-colors" />
        </div>
      </div>
      ...
      {/* List */}
      <div className="flex-1 overflow-y-auto px-2 pb-2" style={{ maxHeight: 300 }}>
        {filtered.length === 0 ? (
          ...
          return (
            <button key={inst.symbol} onClick={() => { onSelect(inst); onClose(); }}
              ...
              style={isSel ? { background: 'rgba(79,195,247,0.08)' } : {}}>
              {/* Favorite heart */}
...
export default function LotCalculatorPage() {
  ...
  useEffect(() => { fetchExchangeRates().then(r => { setExchangeRates(r); setRatesLoaded(true); }); }, []);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-10 animate-fadeIn">
            ...
            <p className="text-kmf-text-secondary max-w-2xl mx-auto text-lg">
              Calculate the exact position size for any trade across <strong className="text-kmf-text-primary">200+ instruments</strong>. Forex, crypto, stocks, indices, commodities — with real-time exchange rates.
            </p>
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="p-6 lg:p-8 space-y-5 lg:border-r border-white/5">
                ...
                <div>
                  ...
                  {!isCustom ? (
                    <div className="relative">
                      ...
                      {showPicker && <SymbolPickerPanel selected={instrument} onSelect={setInstrument} onClose={() => setShowPicker(false)} />}
                    </div>
                  ) : (
                ...
                <div className="flex items-center gap-2 text-xs text-kmf-text-tertiary">
                  <div className={`w-2 h-2 rounded-full ${ratesLoaded ? 'bg-green-400' : 'bg-yellow-400 animate-pulse'}`} />
                  {ratesLoaded ? 'Exchange rates loaded (cached 1h)' : 'Loading exchange rates...'}
                </div>
              </div>
