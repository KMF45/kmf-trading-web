      ...
      <div className="container mx-auto relative z-10">
        ...
        <div className="flex items-center justify-center gap-3 mb-10">
          <button
            ...
            style={{
              background: billing === 'monthly' ? 'rgba(79,195,247,0.15)' : 'transparent',
              color: billing === 'monthly' ? '#4FC3F7' : '#7A9BB0',
              border: billing === 'monthly' ? '1px solid rgba(79,195,247,0.30)' : '1px solid rgba(255,255,255,0.07)',
            }}
          >
            Monthly
          </button>
          <button
            ...
            style={{
              background: billing === 'annual' ? 'rgba(79,195,247,0.15)' : 'transparent',
              color: billing === 'annual' ? '#4FC3F7' : '#7A9BB0',
              border: billing === 'annual' ? '1px solid rgba(79,195,247,0.30)' : '1px solid rgba(255,255,255,0.07)',
            }}
          >
            Annual
            <span
              ...
              {billing === 'monthly' ? (
                ...
                <>
                  <div className="flex items-end gap-1 mb-1">
                    <span className="text-4xl font-extrabold text-kmf-text-primary" style={{ letterSpacing: '-0.03em' }}>
                      <span className="gradient-text">${ANNUAL_MONTHLY.toFixed(2)}</span>
                    </span>
                    <span className="text-kmf-text-tertiary mb-1.5 text-sm">/ month</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-kmf-text-tertiary line-through">${(MONTHLY_PRICE * 12).toFixed(2)}/yr</span>
                    <span className="text-xs font-bold" style={{ color: '#00C853' }}>${ANNUAL_PRICE}/yr</span>
                    <span
                      className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                      style={{ background: 'rgba(0,200,83,0.15)', color: '#00C853', border: '1px solid rgba(0,200,83,0.30)' }}
                    >
                      SAVE ${((MONTHLY_PRICE * 12) - ANNUAL_PRICE).toFixed(2)}
                    </span>
                  </div>
                </>
              )}
              <p className="text-sm text-kmf-text-tertiary mt-2">For traders serious about improvement.</p>
            </div>
